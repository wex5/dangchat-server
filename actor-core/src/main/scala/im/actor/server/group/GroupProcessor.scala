...
private[group] final class GroupProcessor
  ...
  protected def handleQuery: PartialFunction[Any, Future[Any]] = {
    ...
    case GetApiStruct(clientUserId, loadGroupMembers) ⇒
      //添加isShare参数 by Lining 2016/8/17
      val groupIsShare = scala.concurrent.Await.result(getGroupIsShared(state.id), Duration.Inf)
      getApiStruct(clientUserId, loadGroupMembers, groupIsShare)
  ...
  /**
   * 得到群组的共享状态 by Lining 2016/8/17
   *
   * @param groupId
   * @return
   */
  private def getGroupIsShared(groupId: Int): scala.concurrent.Future[Boolean] = {
    db.run(im.actor.server.persist.GroupRepo.groupIsShared(groupId))
  }
